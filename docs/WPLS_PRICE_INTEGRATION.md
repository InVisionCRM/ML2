# WPLS/MORBIUS Price Integration

## Overview
Implemented real-time WPLS to MORBIUS price conversion using the PulseX liquidity pool as the primary source, with DexScreener API as a fallback.
...
1. **Primary Source:** Fetches reserves from PulseX V2 pair (`0x9977e170C9B6E544302E8DB0Cf01D12D55555289`)
   ...
   - Calculates WPLS per MORBIUS accounting for decimal differences (WPLS: 18, MORBIUS: 9)
   - Formula: `(wplsReserve * 1e9) / MORBIUSReserve`
...
**Purpose:** Calculate the exact WPLS amount needed for a given MORBIUS amount

**Parameters:**
- `MORBIUSAmount` - Amount of MORBIUS needed (in base units, 9 decimals)
- `wplsPerMORBIUS` - Price from `useWplsPrice()` hook
- `bufferPercent` - Default 11.1% (5.5% tax + 5% slippage + buffer)

**Calculation:**
```typescript
// Base WPLS needed
const baseWpls = (MORBIUSAmount * wplsPerMORBIUS) / 1e9
...
2. Replaced hardcoded `MORBIUSCost * 2n` with dynamic price calculation
...
- **WPLS/MORBIUS Pair:** `0x9977e170C9B6E544302E8DB0Cf01D12D55555289`
- **WPLS Token:** `0xA1077a294dDE1B09bB078844df40758a5D0f9a27`
- **MORBIUS Token:** `0xB5C4ecEF450fd36d0eBa1420F6A19DBfBeE5292e`
...
4. In the purchase modal, check:
   - The WPLS amount is dynamically calculated (not just 2x MORBIUS)
...
4. **Token ordering:** Correctly identifies which reserve is WPLS vs MORBIUS
5. **Decimal differences:** Properly converts between 18-decimal (WPLS) and 9-decimal (MORBIUS) tokens
